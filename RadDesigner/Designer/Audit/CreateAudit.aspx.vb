...
Public Class CreateAudit
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            istop(Session("MasterCourse"))
            lblWarn.Text = CanAudit(Session("MasterCourse"))
            If lblWarn.Text = "Audit Approved" And istop20(Session("MasterCourse")) = False And getAuditCount(Session("MasterCourse")) = 0 Then
                Button1.Visible = True
            ElseIf lblWarn.Text = "Audit Approved" And istop20(Session("MasterCourse")) = False And getAuditCount(Session("MasterCourse")) = 1 Then
                lblStatus.Visible = True
                lblStatus.Text = "An audif for this course has been scheduled."
            ElseIf lblWarn.Text = "Audit Approved" And istop20(Session("MasterCourse")) = True And getAuditCount(Session("MasterCourse")) <= 1 Then
                Button1.Visible = True
            End If
            '''Form begins with session(Mastercourse) which it inherits from the main page where designer firs selects a course before having access to audit create form
        Else
                ' sesson mastercourse is created fom ddlist coourses on autopostback from selectiong new course
                'Response.Write(Session("MasterCourse") & "--New selected mastercourse <br>")
            End If
    ...
    Sub GetCourses(ByVal myID As Integer)
        'This only fires on pageload before selecting a different mastercourse
        ...
        If dr.HasRows Then
            While dr.Read()
                ...
                If dr("BBlock") = False Then
                    ...
                    mycount = mycount + 1
                End If
                '''For Testing   allow audit; override exceptions above[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[[
                ''mycount = 0
                If mycount = 0 Then
    ...
    Private Function istop20(ByVal mymaster As String) As Boolean
    ...
    Private Function istop(ByVal mymaster As String) As String
        ...
        Else
            lblIsTop.Text = ""
            'lblauditcount.Text = getAuditCount(mymaster) & " =no count"
    ...
    Protected Sub ddlCourses_SelectedIndexChanged(sender As Object, e As EventArgs) Handles ddlCourses.SelectedIndexChanged
        ...
        lblauditcount.Text = getAuditCount(ddlCourses.SelectedValue)
        istop(ddlCourses.SelectedValue)
        lblWarn.Text = CanAudit(ddlCourses.SelectedValue)
        If lblWarn.Text = "Audit Approved" And istop20(ddlCourses.SelectedValue) = False And getAuditCount(ddlCourses.SelectedValue) = 0 Then
            Button1.Visible = True
        ElseIf lblWarn.Text = "Audit Approved" And istop20(ddlCourses.SelectedValue) = False And getAuditCount(ddlCourses.SelectedValue) = 1 Then
            lblStatus.Visible = True
            lblStatus.Text = "An audif for this course has been scheduled."
        ElseIf lblWarn.Text = "Audit Approved" And istop20(ddlCourses.SelectedValue) = True And getAuditCount(ddlCourses.SelectedValue) <= 1 Then
            Button1.Visible = True
        End If
