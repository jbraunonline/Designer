...
Public Class AsProctoredExam
    ...
    Protected Sub DoesExamExist(ByVal myMaster As String)
        ...
        If dr.HasRows Then
            ...
            End While
        Else
            Response.Write("<br><font color=red>No Master Proctor Exam record exists for this course.  Please see administrator.</font>")
            CreateExamRecord.Visible = True
            'Response.End()
        End If
    ...
    Protected Sub CreateExamRecord_Click(sender As Object, e As EventArgs) Handles CreateExamRecord.Click
        Dim strsql As String = ""
        Dim mycourse As String = Session("MasterAuditName")
        mycourse = Replace(mycourse, Session("NextTerm"), "")
        mycourse = Replace(mycourse, "_MC_VC", "")
        mycourse = Replace(mycourse, "_", "")
        strsql += "Insert into OL_MasterExam_BU ( [MasterName] ,[Course] ,[Term]) Values  ('" & Session("MasterAuditName") & "','" & mycourse & "','" & Session("NextTerm") & "')"
        Response.Write(strsql)
    End Sub
