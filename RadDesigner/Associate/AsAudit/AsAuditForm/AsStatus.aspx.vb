...
Public Class AsStatus
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Page.IsPostBack Then
            'Determine iif there are multiple audits. if Both audits are not complete, a Sweep cannot be created.
            lblMasterName.Text = Session("MasterAuditName")  ' get MasterName of course
            lblAuditNo.Text = getAuditCount(Session("MasterAuditName"))  'get count of audits for this course
            LblIsComplete.Text = getAuditComplete(Session("MasterAuditName")) ' get count of complete audits for this course.
            Dim auditcount As Integer = CInt(lblAuditNo.Text)
            Dim auditcompletecount As Integer = CInt(LblIsComplete.Text)
            If auditcount = auditcompletecount Then
                lblReleaseNotice.Text = "All audits are complete"
            ElseIf auditcount > 1 And auditcount <> auditcompletecount Then
                lblReleaseNotice.Text = "This course has multiple audits and a sweep should not be created until all audits for this course are complete."
            End If


            'Does OL_AuditStatus record exist

            'NOTE check if audit status exists for this auditID, not Mastername.....replaced masterAuditName with auditID 3.3.20
            ...
            'get the status of this audit; 1-complete etc   this determines if the complete&create sweep button appears.
            Dim mystatus As String = GetStatus(Session("Audit_ID"))
            lblMasterStatus.Text = mystatus

            'getdata = loads dateEntered for this audit
            getData()

            'Find out if there is a sweep generated, if so then show on form
            If isSweep(Session("MasterAuditName")) = True Then
                lblIsSweep.Text = "Sweep created"
            Else
            End If

            'Now show the correct update or save buttons appear
        ...
        'FInally, if the audit record is complete, show pushbakd and create sweep buttons
        If Session("Role") = 1 Then
            ...
            If lblMasterStatus.Text = "1-Complete" Then
    ...
    Private Function getAuditComplete(ByVal mymaster As String) As Integer
        ...
        'Return myreturn
    End Function
    Protected Function DoesAuditStatusExist(ByVal AuditID As String) As Boolean
        ...
        Dim mySQL As String = "Select * from OL_AuditStatus where Audit_ID = " & AuditID
        Dim myConnection As New SqlConnection(ConfigurationManager.ConnectionStrings("DesignConnectionString").ConnectionString)
    ...
    Public Function isSweep(ByVal myCourse As String) As Boolean
        'Is there a sweep for this mastercourse
        Dim myConnection As New SqlConnection(ConfigurationManager.ConnectionStrings("DesignConnectionString").ConnectionString)
    ...
    Protected Sub getData()
        'this gets the last date this audit was saved.
    ...
    Protected Sub UpdateData(ByVal myStatus As String)  'this updates the OL_AuditStatus record for this page  status/date
    ...
    Protected Sub btnSweep_Click(sender As Object, e As EventArgs) Handles btnSweep.Click
        ...
        If isSweep(Session("MasterAuditName")) = False Then
            ...
            lblIsSweep.Text = "Sweep Created"
        Else
            lblIsSweep.Text = "Sweep for " & Session("MasterAuditName") & " exists."
        End If
    End Sub
