<%@ Page Language="vb" AutoEventWireup="false" CodeBehind="NotReleasedReport.aspx.vb" Inherits="RadDesigner.NotReleasedReport" %>
...
<body>
    <form id="form1" runat="server">
      ...
      <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:DesignConnectionString %>" SelectCommand="SELECT OL_MasterCourse.MasterName, OL_MasterCourse.Developer_SME, OL_MasterCourse.Instructional_Designer, OL_MasterCourse.Phase, 
OL_MasterAudit.AuditComplete, OL_MasterSweep.SweepComplete, OL_MasterRelease.Datecreated, OL_MasterRelease.Daterelease, OL_MasterCourse.Course 
FROM OL_MasterCourse 
LEFT OUTER JOIN OL_MasterRelease ON OL_MasterCourse.MasterName = OL_MasterRelease.MasterName 
LEFT OUTER JOIN OL_MasterSweep ON OL_MasterCourse.MasterName = OL_MasterSweep.MasterName 
LEFT OUTER JOIN OL_MasterAudit ON OL_MasterCourse.MasterName = OL_MasterAudit.MasterName 
WHERE (OL_MasterCourse.Term = @Term) AND (OL_MasterCourse.Status = 'Active')
and OL_MasterCourse.Course in (select Course from BB_97_NotReleased)
ORDER BY OL_MasterCourse.Course" DeleteCommand="DELETE FROM [Workflow] WHERE [ID] = @ID" InsertCommand="INSERT INTO [Workflow] ([CourseID], [Instructional_Designer], [Textbook], [Exam], [Audit], [Sweep], [Copy], [Release]) VALUES (@CourseID, @Instructional_Designer, @Textbook, @Exam, @Audit, @Sweep, @Copy, @Release)" UpdateCommand="UPDATE [Workflow] SET [CourseID] = @CourseID, [Instructional_Designer] = @Instructional_Designer,  [CourseInit] = @CourseInit,, [Textbook] = @Textbook, [Exam] = @Exam, [Audit] = @Audit, [Sweep] = @Sweep, [Copy] = @Copy, [Release] = @Release WHERE [ID] = @ID">
        ...
        <span class="auto-style2"><strong>Not Released Courses for Term
        </strong></span><strong>
        <asp:Label ID="lblTerm" runat="server" CssClass="auto-style2"></asp:Label>
        &nbsp;
        <asp:Button ID="btnRefresh" runat="server" BorderColor="#993333" BorderWidth="3px" Font-Bold="True" Text="Refresh Data " Visible="False" />
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        </strong>
...
<GroupingSettings CollapseAllTooltip="Collapse all groups"></GroupingSettings>
    ...
    <p>
        NOTE: to eliminate duplicate courses, you should add to SQL: AND (OL_MasterAudit.isPrimary = 1)</p>
    <p>
        Currently any courses that co not have a primary autid do not show, if you remove the OL_MasterAudig.IsPrimary qualifier, courses without a primary audit do not show.</p>
</body>
</html>
